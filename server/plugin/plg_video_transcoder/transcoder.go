...
func transcodeVideoSegment(cachePath string, segmentNumber int, w io.Writer) error {
	p, err := NewPipeline(cachePath, astiav.MediaTypeVideo, "libx264")
	if err != nil {
		return err
	}
	defer p.Close()

	// Output dimensions: scale=-2:720 without upscaling, width rounded up to even.
	p.encCtx.SetHeight(min(720, p.decCtx.Height()))
	p.encCtx.SetWidth((p.decCtx.Width()*p.encCtx.Height()/p.decCtx.Height() + 1) &^ 1)
	...
	if err := p.Build(FilterGraphSpec{
		...
		graphSpec: fmt.Sprintf("scale=%d:%d,format=yuv420p", p.encCtx.Width(), p.encCtx.Height()),
	}); err != nil {
		return err
	}
	return p.Run(
