...
*Prerequisites*: Git, Make, Go, and a C toolchain for CGO (for example `gcc`).
...
# Prepare generated files and dependencies
make init

# Build binary (or run `make all`)
make build
...
- `make init`: fetch Go dependencies and run code generation
- `make build`: build `dist/filestash` with `fts5` tag
- `make all`: run `init` then `build`
